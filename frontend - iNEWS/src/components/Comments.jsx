...
const Comments = ({comment}) => {
  return (
    <div className="mt-2 text-black dark:text-gray-200 antialiased flex ">
      <img className="rounded-full h-8 w-8 mr-2 mt-1 " src={comment.commentor.pictures !== null ? comment.commentor.pictures : userImage} />
      <div>
        <div className="bg-gray-100 dark:bg-gray-700 rounded-3xl px-4 pt-2 pb-2.5">
          <div className="font-semibold text-sm leading-relaxed">{comment.commentor.email}</div>
          <div className="text-normal leading-snug md:leading-normal"
          >{comment.text}</div>
        </div>
        <div className="text-sm ml-4 mt-0.5 text-gray-500 dark:text-gray-400">{comment.comment_at}</div>
